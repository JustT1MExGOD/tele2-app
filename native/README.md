...
- Клиент для Windows: только x64, проверялось на Windows 11. Установщик не подписан цифровой подписью Authenticode — политика проверки обновлений `warn` допускает отсутствие подписи, но блокирует повреждённую. На жёстко управляемых корпоративных ПК из-за этого возможна блокировка SmartScreen/антивирусом при первом запуске каждой новой версии (репутация Windows SmartScreen считается по хэшу конкретного файла, а не по программе в целом) — постоянное решение это код-подпись сертификатом (лучше EV), временный обход — просить IT занести хэш конкретной версии в исключения или запустить один раз под администраторской учёткой.
