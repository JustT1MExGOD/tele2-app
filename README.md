...
![version](https://img.shields.io/badge/version-19.20.0-2AABEE?style=flat-square)
...
**Актуальная версия клиента:** `19.20.0`  
...
### После 19.2 — UX-батчи и security-аудит ✅ частично (19.3.0-19.20.0)
...
(19.17.0); TypeBox-валидация начата на самых денежных/расписательных
write-роутах — продажи, касса, график, смены (19.18.0), продолжена на
доступ/роли и CRUD сотрудников/точек (19.19.0, попутно пойман и закрыт
реальный regression с ajv-коэрсией null→0 на координатах геолокации);
продолжена на задачах, поддержке, промокодах, объявлениях и каналах сети
(19.20.0). Оставшиеся пункты того же ревью — TypeBox на ~15 оставшихся
write-роутах (отчёты, метрики, план-факт, live-alerts, what-if), слой
доступа к данным, frontend TypeScript+Vite, полный audit trail, кэш
supervisor-scope и т.п. — крупнее по объёму и ждут отдельного приоритетного
захода, не сделаны одним махом умышленно.
...
- TypeBox-схемы (`@sinclair/typebox`, 19.18.0) на `schema.body` роута —
  ...
  никаких новых отказов на легитимных запросах. Первый заход — 10 самых
  денежных/расписательных write-роутов (§21, 19.18.0), затем доступ/роли и
  CRUD сотрудников/точек (19.19.0), затем задачи/поддержка/промокоды/
  объявления (19.20.0). Динамические по форме тела (кастомные метрики
  продаж, разнородные `sync/batch`-операции) сознательно оставлены
  `additionalProperties: true` — схема не должна быть строже уже отлаженной
  ручной логики внутри обработчика.
...
*README · актуально на v19.20.0 · август 2026*
