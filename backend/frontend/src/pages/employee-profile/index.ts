/**
 * 21.x (Frontend rewrite continuation) — fourth migrated legacy page,
 * replacing frontend/js/18-employee-profile.js file-for-file. A real
 * router.ts page like reports.js/alerts.js, but the legacy nav dispatch
 * (02-nav-utils.js) calls it by a DIFFERENT bare name than the usual
 * loadXPage() convention — `renderEmployeeProfile()` directly, no `load`
 * prefix — so the bridge here matches that exact name instead of
 * introducing a new one 02-nav-utils.js would need editing for.
 *
 * openEmployeeProfile(employeeId) is the entry point OTHER legacy files
 * call (06-team-bfq.js, 14-command-center.js) — bridged onto window.* so
 * those callers keep working unchanged, same as any other cross-file
 * global. It only sets state + switchPage(); it deliberately does NOT
 * call the render function directly (see comment on the legacy original)
 * to avoid recursing through switchPage().
 *
 * bfq/gamification/shifts.recent come back loosely typed
 * (Record<string, unknown> / unknown[]) from EmployeeProfileResponse —
 * same acknowledged pattern as the rest of this API surface for
 * big, ad-hoc dashboard payloads (see README §21, 20.7.0). Cast locally
 * rather than fighting it with narrower types nothing else needs.
 */
import { registerPage, renderPage } from '../../app/router.js';

let currentEmployeeProfileId: number | string | null = null;

const EMPLOYEE_HEALTH_COMPONENT_LABEL: Record<string, string> = {
  plan: 'План',
...
export function openEmployeeProfile(employeeId: number | string): void {
  currentEmployeeProfileId = employeeId;
  switchPage('employee-profile');
}

export async function renderEmployeeProfilePage(): Promise<void> {
  ...
  try {
    const d: any = await window.apiClient.getEmployeeProfile(authHeaders(), employeeId, orgQueryParam());

    const tone = commandCenterTone(d.health?.score || 0);
    const comps: Record<string, { value: number }> = d.health?.components || {};
    const gam: any = d.gamification || {};
    const bfq: any = d.bfq || {};
    const shifts: any[] = (d.shifts && d.shifts.recent) || [];
    ...
    box.innerHTML = `
      ...
      <div class="section">
        ...
        ${shifts
          ...
          : '<div class="section"><div class="empty">Нет закрытых смен за период</div></div>'
      }
    `;
  } catch (e) {
    console.error(e);
...
registerPage('employee-profile', renderEmployeeProfilePage);

declare global {
  interface Window {
    openEmployeeProfile: typeof openEmployeeProfile;
    renderEmployeeProfile: () => void;
  }
}
window.openEmployeeProfile = openEmployeeProfile;
window.renderEmployeeProfile = () => {
  renderPage('employee-profile');
};
