 ...
 * frontend/js/10-tutorial.js file-for-file: interactive onboarding state
 * machine (cutscene/coach/practice-real steps, employee + manager tracks).
 *
 * Every top-level function is window-bridged, including ones the original
 * classic script never bridged explicitly (nextTutorialStep, skipTutorial,
 * onTutPractice) — those were only reachable there because top-level
 * `function` declarations in a classic <script> become window properties
 * automatically; a module needs the explicit bridge. index.html's static
 * onclick="nextTutorialStep()"/"skipTutorial()" markup depends on this.
 */

interface TutorialPracticeOption {
  id: string;
  label: string;
}

interface TutorialStep {
  kind: 'cutscene' | 'coach' | 'practice-real';
  chapter?: number;
  page?: string;
  title: string;
  text: string;
  highlight?: string;
  task?: string;
  practice?: TutorialPracticeOption[];
...
const TUTORIAL_EMPLOYEE: TutorialStep[] = [
  {
    kind: 'cutscene',
    chapter: 1,
    title: 'Здоро́во!',
...
const TUTORIAL_MANAGER: TutorialStep[] = [
  {
    kind: 'cutscene',
    chapter: 1,
...
let tutorialMode: 'employee' | 'manager' = 'employee';
let tutorialSteps: TutorialStep[] = TUTORIAL_EMPLOYEE;
let tutorialStepDone = false;
let practiceHits = new Set<string>();
let tutTapHandler: ((ev: MouseEvent) => void) | null = null;

function stepChapters(): string[] {
  return tutorialMode === 'manager' ? CHAPTERS_MANAGER : CHAPTERS_EMPLOYEE;
}

function needsAction(step: TutorialStep): boolean {
  return step.kind === 'coach' || step.kind === 'practice-real' || !!(step.practice && step.practice.length);
}

export function startTutorial(mode: string): void {
...
export function startManagerTutorial(): void {
...
export function skipTutorial(): void {
  ...
  if (tutorialMode === 'employee' && !localStorage.getItem('t2_tutorial_done')) {
    ...
    try {
      (window as any).tg?.HapticFeedback?.notificationOccurred?.('error');
    } catch (_) {}
    return;
  }
  finishTutorial(true);
...
function markStepDone(msg?: string): void {
  ...
  if (step && step.kind === 'coach') {
    ...
    if (task) {
      ...
      if (msg)
        task.textContent =
          '<svg class="ic" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" > <path d="M20 6 9 17l-5-5" /> </svg> ' + msg;
    }
    const nextBtn = document.getElementById('tutNextBtn') as HTMLButtonElement | null;
    if (nextBtn) nextBtn.disabled = false;
  } else {
    const nextBtn = document.getElementById('tsNextBtn') as HTMLButtonElement | null;
    if (nextBtn) nextBtn.disabled = false;
    if (msg) toast(msg, 'ok');
  }
  try {
...
function renderCutsceneStep(step: TutorialStep): void {
  ...
  if (chapterLabel) {
    chapterLabel.textContent = step.chapter ? `Глава ${step.chapter} из ${chapters.length} · ${chapters[step.chapter - 1] || ''}` : 'Экзамен';
  }

  const tsTitle = document.getElementById('tsTitle');
  if (tsTitle) tsTitle.textContent = step.title;
  const tsText = document.getElementById('tsText');
  if (tsText) tsText.textContent = step.text;
  ...
  if (pracEl) {
    if (step.kind === 'practice-real') {
      ...
      pracEl.style.display = 'flex';
      pracEl.innerHTML = step.practice.map((p) => `<button type="button" data-pid="${p.id}" onclick="onTutPractice('${p.id}')">${p.label}</button>`).join('');
    } else {
  ...
  if (dots) {
    dots.innerHTML = tutorialSteps.map((_, i) => `<div class="tut-dot ${i === tutorialIndex ? 'on' : ''}"></div>`).join('');
  }
  ...
  const skipBtn = document.getElementById('tsSkipBtn') as HTMLElement | null;
...
function renderCoachStep(step: TutorialStep): void {
  ...
  const nextBtn = document.getElementById('tutNextBtn') as HTMLButtonElement | null;
  const skipBtn = document.getElementById('tutSkipBtn') as HTMLElement | null;
  ...
  const practiceEl = document.getElementById('tutPractice');
  ...
  if (step.highlight) {
    tutTapHandler = function (ev: MouseEvent) {
      const el = document.querySelector(step.highlight!);
      if (!el) return;
      const nav = document.querySelector('.bottom-nav');
      const target = ev.target as Node;
      const hit = el === target || el.contains(target) || (targetBottom && nav && nav.contains(target) && (target as HTMLElement).closest?.(step.highlight!));
      if (hit) {
        setTimeout(() => {
...
export function onTutPractice(pid: string): void {
  const step = tutorialSteps[tutorialIndex];
  if (!step) return;
...
/** Открыть настоящую форму продажи в тренировочном режиме —
 * features/add-sale проверяет window.__tutorialDryRun и не пишет реальный
 * POST /sales. */
export function beginPracticeReal(): void {
  ...
  try {
    ...
    console.warn('tutorial openAddSale failed', e);
  }
}
...
declare global {
  interface Window {
    maybeOfferTutorial: typeof maybeOfferTutorial;
    startTutorial: typeof startTutorial;
    startManagerTutorial: typeof startManagerTutorial;
    beginPracticeReal: typeof beginPracticeReal;
    nextTutorialStep: typeof nextTutorialStep;
    skipTutorial: typeof skipTutorial;
    onTutPractice: typeof onTutPractice;
  }
}
...
window.onTutPractice = onTutPractice;
