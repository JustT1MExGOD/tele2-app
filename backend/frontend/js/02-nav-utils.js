/* 02-nav-utils.js — часть T2 Sales Mini App (см. index.html).
   Классический скрипт, общая глобальная область со всеми /js/*.js — порядок подключения важен. */
    ...
    /* 12: свайп между вкладками нижней навигации — лёгкий вариант (жест
       переключает вкладку с анимацией, без живого перетаскивания за
       пальцем, как в initSwipePanels выше — полноценная карусель
       потребовала бы переделать скролл всех вкладок с document-level на
       постраничный, отдельная задача). Один глобальный набор слушателей
       на document, а не на конкретную страницу — переживает switchPage(). */
    function initTabSwipe() {
      let startX = 0, startY = 0, dragging = false, horizontal = null;

      function modalOrTutorialOpen() {
        return document.getElementById('overlay')?.classList.contains('show')
          ...
          || document.getElementById('tutorialScreen')?.classList.contains('show');
      }

      // Не перехватываем жест, если он начался внутри реально
      // прокручиваемой по горизонтали области (BFQ-таблица, heatmap) —
      // иначе перетаскивание такой области будет промахиваться в свайп вкладки.
      function startedInHorizontalScroll(el) {
        let node = el;
        while (node && node !== document.body) {
          if (node.scrollWidth > node.clientWidth) {
            const of = getComputedStyle(node).overflowX;
            if (of === 'auto' || of === 'scroll') return true;
          }
      ...
      // Список вкладок — из видимой сейчас нижней навигации (обычная
      // 5-вкладочная или supervisor-shell), а не хардкод — тогда работает
      // для обеих без дублирования списка.
      function visibleTabNames() {
        const nav = Array.from(document.querySelectorAll('nav.bottom-nav'))
          .find(n => getComputedStyle(n).display !== 'none');
        if (!nav) return [];
        return Array.from(nav.querySelectorAll('.nav-item[data-page]')).map(n => n.dataset.page);
      }

      // switchPage() меняет .active синхронно (display:none↔block) — старая
      // страница пропадает из раскладки раньше, чем успела бы сыграть
      // «исходящая» transition, поэтому анимируем только вошедшую страницу.
      function switchPageWithSlide(name, direction) {
        switchPage(name);
        const el = document.querySelector('.page.active');
        if (!el) return;
        const inClass = direction === 'left' ? 'page-slide-in-left' : 'page-slide-in-right';
        el.classList.add(inClass);
        el.addEventListener('animationend', () => el.classList.remove(inClass), { once: true });
      }

      document.addEventListener('touchstart', (e) => {
        if (modalOrTutorialOpen()) { dragging = false; return; }
        if (startedInHorizontalScroll(e.target)) { dragging = false; return; }
        startX = e.touches[0].clientX;
        startY = e.touches[0].clientY;
        dragging = true;
        horizontal = null;
      ...
      document.addEventListener('touchend', (e) => {
        ...
        const tabs = visibleTabNames();
        const idx = tabs.indexOf(page);
        if (idx === -1) return; // drill-in страница вне нижней навигации — жест не активен

        if (dx < 0 && idx < tabs.length - 1) {
          switchPageWithSlide(tabs[idx + 1], 'left');
        } else if (dx > 0 && idx > 0) {
          switchPageWithSlide(tabs[idx - 1], 'right');
        }
      }, { passive: true });
    }
    initTabSwipe();
