...
export async function upsertEmployeeMonthPlanRow(
  ...
  const res = await query(`INSERT INTO employee_month_plans (${cols.join(',')}) VALUES (${ph.join(',')})
    ON CONFLICT(employee_id,month) DO UPDATE SET ${updates},updated_at=now() RETURNING *`,vals);
  return res.rows[0];
}
...
export async function upsertStoreMonthPlanRow(storeId: string, monthStartDate: string, norm: Record<string, number>): Promise<any> {
  const keys=Object.keys(norm).filter(k=>/^[a-z][a-z0-9_]{0,29}$/.test(k));
  const cols = ['store_id', 'month', ...keys];
  const ph = cols.map((_, i) => `$${i + 1}`);
  const vals: any[] = [storeId, monthStartDate, ...keys.map((m) => norm[m])];
  const updates = keys.map((m) => `${m} = EXCLUDED.${m}`).join(', ');
  const res = await query(
...
export async function materializeRow(storeId: string, date: string, p: Record<string, number>): Promise<void> {
  ...
  await query(`INSERT INTO store_plans(store_id,plan_date,${keys.join(',')}) VALUES($1,$2,${keys.map((_,i)=>'$'+(i+3)).join(',')})
    ON CONFLICT(store_id,plan_date) DO UPDATE SET ${keys.map(k=>`${k}=EXCLUDED.${k}`).join(',')}
    WHERE (${keys.map(k=>'store_plans.'+k).join(',')}) IS DISTINCT FROM (${keys.map(k=>'EXCLUDED.'+k).join(',')})`,[storeId,date,...keys.map(k=>num(p[k]))]);
}
