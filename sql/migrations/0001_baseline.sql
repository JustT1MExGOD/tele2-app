-- 0001_baseline.sql
--
-- Снапшот прод-схемы на момент введения системы миграций (эпик 17.0,
-- 17.5.0) — раньше схема правилась ad hoc SQL прямо на Railway, без
-- какого-либо трекинга. На реальном проде этот файл НЕ исполняется (там
-- всё это уже есть) — вместо этого схема_migrations сразу засеяна записью
-- '0001_baseline.sql', см. README. Исполняется только на чистой БД
-- (CI, локальный тест) — там воспроизводит текущую прод-схему с нуля.
-- Дальше — только новые пронумерованные файлы, никогда не редактировать
-- уже применённый файл задним числом.
--
