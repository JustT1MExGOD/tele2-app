...
- **AAL2** — конкретный channel-контекст РЕАЛЬНО прошёл MFA: для
  browser/phone — `employee_sessions.mfa_verified_at`, выставляется при
  выдаче сессии через MFA-ветку логина; для Telegram (сессии нет вообще,
  ADR-005) — короткоживущий (12ч) server-side грант
  (`mfa_telegram_grants`, `auth/mfa/telegram-grant.ts`), выдаваемый ТОЛЬКО
  после реальной проверки фактора через `POST /auth/mfa/telegram/verify`.
  **20.53.0** — раньше для Telegram AAL2 засчитывалось "у аккаунта есть
  настроенный фактор", без его реальной проверки в текущем доступе; это
  позволяло украденному Telegram-устройству/сессии получить privileged-
  доступ без предъявления второго фактора атакующим — см.
  [ADR/009, "20.53.0 revision"](./ADR/009-mfa-step-up.md). Оба канала
  теперь требуют один и тот же класс доказательства.
  `auth/assurance.ts::checkPrivilegedAssurance()` возвращает
  `mfa_enrollment_required` (фактора нет вообще) отдельно от
  `mfa_reverification_required` (фактор есть, но ЭТОТ конкретный
  channel-контекст его не подтверждал — сессия/грант выданы до
  enrollment, истекли, или ещё не запрашивались).
- **AAL3** — свежий step-up (см. ниже) для конкретного опасного действия
  прямо сейчас, не "было пройдено когда-то в этой сессии/гранте".
...
(20.52.1, закрыто для обоих каналов 20.53.0)** — `auth/guards.ts::requireActive()`
блокирует `403 mfa_enrollment_required`/`mfa_reverification_required` для
admin/supervisor без реального AAL2 (см. модель выше) на КАЖДОМ защищённом
роуте, кроме явно помеченных enrollment/status/logout
(`{allowMfaEnrollment:true}`) — иначе enrollment стал бы недостижим.
Вычисляется один раз в `authPlugin` (`auth/assurance.ts`) — для Telegram
теперь дополнительно резолвит AAL2-грант (см. модель выше), только когда
роль вообще MFA-mandatory (не тратит лишний DB-запрос на обычных
сотрудников). Раньше (20.52.0) mandatory обеспечивался только тем, что
step-up физически недостижим без фактора — обычные privileged-функции (не
step-up-gated) оставались доступны на голом AAL1; это было осознанно
отклонённой альтернативой ("блокировка всего API" виделась слишком
широкой), но оказалось недостаточным для реального требования "MFA
обязателен". См. [ADR/009, раздел "20.52.1 revision"](./ADR/009-mfa-step-up.md)
и ["20.53.0 revision"](./ADR/009-mfa-step-up.md) для Telegram-части.

**Route Auth Policy ratchet (CI, 20.53.0)** — `check:route-auth`
(`scripts/check-route-auth-policy.mjs`) сканирует каждую регистрацию
роута в `api/routes/**` и падает, если роут не вызывает ни один
централизованный guard И не находится явно в allowlist'е намеренно
публичных/pre-auth роутов (с обязательной причиной на строку) — тот же
класс инструмента, что `check-no-direct-sql.mjs`. Не позволяет новому
business-роуту повторить класс пробела, найденный этим же аудитом
(`requireAuth` вместо `requireActive`, custom-резолверы в обход
централизованной политики) — молча, без явного code-review решения.
...
конкретному channel-контексту, если запрос его нёс
(`mfa_step_up_tickets.session_token_hash`) — украденный тикет не
работает из другого контекста того же сотрудника; изначально только
browser/phone-сессия, с 20.53.0 — и Telegram AAL2-грант тоже (см.
модель выше). Запрос без ни одного из двух (не должно происходить —
requireActive уже требует AAL2 для попадания на `/auth/mfa/step-up`
в принципе) оставляет тикет employee-scoped, как раньше.
...
| Admin Center (`/admin/*` — коррекции продаж/смен/графика/планов, флаги функциональности, операционный центр по всем сетям) | ❌ | ❌ | ❌ | ❌ | ✅ (`requireAdmin`) |
