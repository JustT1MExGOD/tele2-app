...
Рендер требует все файлы в `backend/assets/fonts/` — при отсутствии
resvg падает на этапе рендера, не деградирует тихо:

| Файл | Используется в |
|---|---|
| `DejaVuSans.ttf` / `DejaVuSans-Bold.ttf` | Базовый рендер (`buildDailyReportPng`) |
| `GoogleSans-Regular.ttf` / `-SemiBold.ttf` / `-Bold.ttf` | Итог-альбом (`buildStoryReportPngs`) |
| `OFL.txt` | Лицензия шрифтов (не используется рендером, просто рядом) |

## Лимиты

- Пул воркеров — **2** (`POOL_SIZE` в `svg-pool.ts`), не конфигурируется
  переменной окружения.
- Явного timeout на рендер в коде нет — зависший воркер не оборвётся сам
  по таймеру; при подозрении на зависание — `railway logs` на отсутствие
  ожидаемого `✅`/`❌` после запуска задачи, не ждать вечно.
- Явного ограничения на размер PNG в коде нет — источник размера
  практически целиком типографика (текст+число элементов на кадре), не
  входные данные пользователя.

## Отладка локально без Telegram

Отправка в чат — отдельный шаг ПОСЛЕ рендера (`notifyChatPhoto`/
`notifyChatMediaGroup`), рендер можно вызвать напрямую и сохранить
результат на диск, не трогая бота:

```ts
import fs from 'node:fs';
import { buildDailyReportPng } from '../core/reports/image.js';

const { png } = await buildDailyReportPng(storeId, date, { kind: 'micro', hourLabel: '14:00' });
fs.writeFileSync('./debug-report.png', png);
```
