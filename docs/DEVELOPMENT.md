...
- [Цикл разработки](#цикл-разработки)
  - [Как проверить, что пуш реально докатился до прода](#как-проверить-что-пуш-реально-докатился-до-прода)
- [Тесты](#тесты)
...
### Как проверить, что пуш реально докатился до прода

`git push` не означает, что изменения сразу видны на
`tele2-app-production.up.railway.app` — CI, сборка и деплой идут уже
**после** пуша, асинхронно. Проверить прогресс:

```bash
railway status
```

Строка `status:` в выводе проходит по стадиям (наблюдалось на практике,
занимает суммарно **около 4-5 минут** от пуша до полного `Online`):

```
● Online · Waiting for CI (1m)   ← CI ещё выполняется
● Online · Building (3m)         ← Railway собирает образ
● Online · Deploying (4m)        ← новый образ разворачивается
● Online                         ← без суффикса — деплой полностью завершён
```

Если тестируете изменение веба через настоящий Telegram Mini App (не
локальный дев-сервер) — после того, как статус вернулся к простому
`● Online`, **полностью закройте и заново откройте** Mini App в Telegram,
а не просто сверните: WebView Telegram может отдать закэшированную старую
версию страницы даже после успешного деплоя (см.
[CHAT.md#особенности-telegram-mini-app-webview](./CHAT.md#особенности-telegram-mini-app-webview)).

Требует `railway login` и привязку к проекту (`railway link`) — разовая
настройка окружения, не делается на каждый деплой.
