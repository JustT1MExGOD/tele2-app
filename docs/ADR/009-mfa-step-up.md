<a id="009--mfa-webauthn--totp--recovery-codes-и-channel-agnostic-step-up"></a>

# 009 — MFA и дополнительное подтверждение независимо от канала
...
- [Уточнение 20.52.1: усиление подтверждения личности](#уточнение-20521-усиление-подтверждения-личности)
- [Уточнение 20.53.0: разрешение AAL2 для Telegram](#уточнение-20530-разрешение-aal2-для-telegram)
...
<a id="библиотеки--только-vetted-ничего-самописного"></a>
...
<a id="login-time-второй-фактор--свойство-аккаунта-не-роли"></a>

### Второй фактор при входе: свойство аккаунта
...
<a id="step-up--channel-agnostic-opaque-ticket-не-session-freshness"></a>

### Дополнительное подтверждение: отдельный непрозрачный токен
...
<a id="last-factor-removal-guard-mfa-3"></a>

### Запрет удаления последнего фактора (MFA-3)
...
<a id="session-hardening-заодно"></a>

### Дополнительные ограничения сессий
...
| WebAuthn-only (без TOTP) | ❌ отклонено | Бриф явно требует TOTP как совместимый резервный вариант — не у каждого privileged-пользователя есть passkey-совместимое устройство сразу |
...
- 35+ новых тестов (unit-style + isolation через реальные роуты/Postgres):
  ...
  отклонение подтверждений другого сотрудника или другой операции, запрет удаления последнего фактора, admin MFA
  reset.
...
<a id="20521-revision--auth-assurance-hardening"></a>

## Уточнение 20.52.1: усиление подтверждения личности
...
сотрудника; TOTP-секрет больше не имеет открытый текст-фолбэка при
...
<a id="20530-revision--telegram-aal2-grant-full-security--reliability-hardening"></a>

## Уточнение 20.53.0: разрешение AAL2 для Telegram
