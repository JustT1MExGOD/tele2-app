<a id="008--e2ee--device-identity--ratchet--post-quantum-planned-не-реализовано"></a>

# 008 — Сквозное шифрование и ключи устройств: запланировано
...
- [Контекст](#контекст)
- [Решение](#решение)
- [Статус по каждому компоненту (§50 отчётный формат)](#статус-по-каждому-компоненту-50-отчётный-формат)
- [Альтернативы](#альтернативы)
- [Что стало бы триггером пересмотра](#что-стало-бы-триггером-пересмотра)
...
| Кандидат | Кто читает открытый текст сегодня | Вывод |
|---|---|---|
...
|---|---|---|
| X25519 / X3DH-class handshake | НЕ РЕАЛИЗОВАНО | Нет private channel, который бы его использовал |
| ML-KEM / post-quantum hybrid | НЕ РЕАЛИЗОВАНО | Ни один зрелый Node/browser-стек не был даже оценён — блокер продуктовый, не библиотечный |
| Device cryptographic identity | НЕ РЕАЛИЗОВАНО | `employee_id`/`telegram_id`/session token остаются НЕ криптографической identity — правильно, но отдельного device-key концепта нет |
| Double Ratchet / per-message защита прошлых сообщений при раскрытии текущего ключа | НЕ РЕАЛИЗОВАНО | Требует handshake выше |
| Device revocation API | НЕ РЕАЛИЗОВАНО | Не нужен без device identity |
| Конвертное шифрование на уровне приложения (Level 2) | ✅ РЕАЛИЗОВАНО | См. ADR-007 — другой, реально нужный слой |
...
| Явно задокументировать ЗАПЛАНИРОВАНО со честным объяснением блокера, вернуться к вопросу, если появится реальная фича приватных сообщений | ✅ принято | Соответствует и STOP condition брифа, и практике проекта (`docs/ADR/002` — та же логика «нет реального триггера, не строим Redis заранее») |
...
- До этого момента раздел остаётся ЗАПЛАНИРОВАНО — не «отменено», а «нет
