...
> **Архив планирования.** Перенесено из README исходного проекта. Отметки выполнения относятся к моменту соответствующей записи. Этот документ не является новым обещанием сроков или независимым подтверждением готовности функций.
...
**Core усиление надёжности (20.8-20.22)** ниже; всё, что уже сделано (эпохи 17-20) —
...
- [Эпоха 17 — «Надёжность перед деньгами» ✅ (17.0.0-17.17.0)](#эпоха-17--надёжность-перед-деньгами--1700-17170)
- [Эпоха 18 — управление сетью ✅ (18.0.0–18.11.0)](#эпоха-18--управление-сетью--180018110)
- [Эпоха 19 — аналитика: выполнено частично (19.1.0–19.2.0)](#эпоха-19--аналитика-выполнено-частично-19101920)
- [После 19.2 — удобство и аудит защиты: выполнено частично (19.3.0–19.21.0)](#после-192--удобство-и-аудит-защиты-выполнено-частично-193019210)
- [Фундамент данных и интерфейса: план владельца (19.22.0 → 20.0.0) ✅](#фундамент-данных-и-интерфейса-план-владельца-19220--2000-)
- [Эпоха 20 — фундамент интерфейса ✅ (20.0.0–20.7.0)](#эпоха-20--фундамент-интерфейса--20002070)
- [20.8–20.22 — независимость от Telegram: план владельца от 22 августа 2026 года](#2082022--независимость-от-telegram-план-владельца-от-22-августа-2026-года)
...
<a id="эпоха-18--command-center-suite--1800-18110"></a>

## Эпоха 18 — управление сетью ✅ (18.0.0–18.11.0)
...
→ `18.4` Tasks/Action Системные маршруты → `18.5` Store Intelligence (Health Score) →
...
<a id="эпоха-19--intelligence--частично-1910-1920"></a>

## Эпоха 19 — аналитика: выполнено частично (19.1.0–19.2.0)
...
<a id="после-192--ux-батчи-и-security-аудит--частично-1930-19210"></a>

## После 19.2 — удобство и аудит защиты: выполнено частично (19.3.0–19.21.0)
...
Lucide вместо эмодзи + Android-усиление надёжности (19.8), новое оформление карточки
...
<a id="datafrontend-foundation--roadmap-задан-владельцем-продукта-19220--2000-"></a>

## Фундамент данных и интерфейса: план владельца (19.22.0 → 20.0.0) ✅
Крупные пункты того же ревью получили явный план вперёд вместо разбора по
одному без сквозной цели. Все 6 пунктов закрыты:

- **19.22.0 Слой доступа к данным** ✅ старт — `src/repositories/`, org-scoped
...
- **19.23.0 Журнал аудита** ✅ — новая таблица `audit_log`, `services/audit.ts`
...
- **20.0.0 Frontend Foundation** ✅ (**major** — по решению владельца
  ...
  уже работала для Слой доступа к данным (19.22.0) и TypeBox (19.18-19.21).
...
<a id="эпоха-20--frontend-foundation--2000-2070"></a>

## Эпоха 20 — фундамент интерфейса ✅ (20.0.0–20.7.0)
...
<a id="208-2022--core-hardening-разрыв-с-telegram-ограничением-roadmap-владельца-продукта-2026-08-22"></a>

## 20.8–20.22 — независимость от Telegram: план владельца от 22 августа 2026 года
...
- **20.8 Полный слой доступа к данным** ✅ («главный технический релиз» — по
...
- **20.9 Граница аутентификации** ✅ (сужено от исходного «Identity
...
- **20.13 Security усиление надёжности** ✅ — по внешнему security-аудиту (v20.11.1),
...
- ~~**20.19-20.22 Platform Layer** — client-neutral API, Web/PWA,
  ...
  (Telegram Login Widget поверх уже существующей Граница аутентификации,
  20.9.0)
...
- **20.33 Domain Integrity** ✅ — не очередной security-аудит, а вопрос
  ...
  ALTER TABLE — только для чтения проверка прод-БД: 0 строк-сирот по всем пяти
...
- **20.39 Desktop Shell** ✅ — развёрнутая критика 20.38 от владельца
  ...
  следующими шагами того же план развития (см. §21, 20.39.0-20.39.1)
- **20.40 Design Системные маршруты + Home Dashboard** ✅ — владелец продукта прислал
  ...
  система, staged план развития), согласовано в диалоге (10+ уточнений от
...
- **20.41 «Динамика выполнения»** ✅ — владелец продукта: переименовать
  ...
  §21, 20.41.0)
- **20.42 Команда — первая `.data-table`** ✅ (20.42.0-20.42.1) — staged план развития из 20.40
...
- **20.43 Автоудаление сообщений бота** ✅ — не из staged desktop-план развития,
...
- **20.44 Schedule/Plans** ✅ — staged план развития из 20.40 (Part G),
  ...
  (см. §21, 20.44.0)
- **20.45 Command Center + Supervisor** ✅ — staged план развития из 20.40
...
- **20.46 Admin — «максимально функциональный»** ✅ — последний пункт
  staged план развития из 20.40 (Part G: "`.data-table` на orgs/audit"), но
...
- **20.48 Web Security & Trust Layer, часть 1 — Auth & Session
  ...
  `employees.phone` с preflight-проверкой на дубли перед заполнение старых записей. ADR-005
...
- **20.49 Web Security & Trust Layer, часть 2 — Browser Security** ✅ —
  ...
  тестов (attribute-breakout проверен реальной `esc()`, не пропуск действия стабом,
