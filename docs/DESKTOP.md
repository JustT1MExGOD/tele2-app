...
Это не единственный клиент для Windows: есть ещё отдельный нативный клиент на Kotlin/Compose (`native/`, свой собственный интерфейс, не веб-страницы) — см. [native/README.md](../native/README.md). Он не заменяет это Electron-приложение и работает параллельно с ним.
